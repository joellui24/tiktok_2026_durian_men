...
def default_model_path() -> Path:
    """Return the bundled Hybrid FM artifact path."""

    return Path(__file__).resolve().parents[1] / "artifacts" / "fm_model.sqlite3"


def default_linear_model_path() -> Path:
    """Return the repository-local frozen linear artifact path."""

    return Path(__file__).resolve().parents[1] / "artifacts" / "linear_model.sqlite3"
